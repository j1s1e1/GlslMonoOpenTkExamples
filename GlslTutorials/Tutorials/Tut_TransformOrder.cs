...
{
	public class Tut_TransformOrder : TutorialBase
	{
		float globalScale = 0.8f;
		Vector3 globalTranslate = new Vector3(0.1f, 0.1f, 0.1f);

		int transformOrder = 0;
		string[] transformOrderStrings = new string[]
		{
			"scale translate",
			"translate scale",
			"none"
		...
		public override void display()
		{
			...
			if (meshes[currentMesh] != null)
			{
				...
				{
				case 0:
					modelMatrix.Scale(globalScale);
					modelMatrix.Translate(globalTranslate);
					break;
				case 1:
					modelMatrix.Translate(globalTranslate);
					modelMatrix.Scale(globalScale);
		...
		public override String keyboard(Keys keyCode, int x, int y)
		{
			...
			case Keys.D6:
				...
				break;	
			case Keys.D7:
				quaternion = new Quaternion(sqrt2over2, sqrt2over2, 0f, 0f);
				break;	
			case Keys.D8:
				quaternion = new Quaternion(0f, sqrt2over2, sqrt2over2, 0f);
				break;	
			case Keys.D9:
				quaternion = new Quaternion(sqrt2over2, 0f, sqrt2over2, 0f);
				break;	
			case Keys.V:
			...
			case Keys.T:
				transformOrder++;
				if (transformOrder > (transformOrderStrings.Length - 1)) transformOrder = 0;
				result.AppendLine(transformOrderStrings[transformOrder]);
				break;
			}
